...
def run():
    items = Item.objects.filter(images__isnull=False).prefetch_related("images")
    facebook_catalog = []
    for item in tqdm(items):
        row = {
            ...
            "link": f"https://muuch-maaya.com/catalogo/{item.slug()}",
            "image_link": f"{item.images.first().link()}",
            "brand": "Casa Maya",
